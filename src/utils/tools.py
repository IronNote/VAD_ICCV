...
# 하나의 클립(feature)을 고정된 length로 조정하는 데 초점
...
#         return pad(feat, length), clip_length
#     else:
#         split_num = int(clip_length / length) + 1  # 분할 개수 계산
#         for i in range(split_num):
#             slice_feat = feat[i * length:i * length + length, :]
#             if slice_feat.shape[0] < length:  # 부족한 경우 패딩 추가
#                 padding = np.zeros((length - slice_feat.shape[0], feat.shape[1]))
#                 slice_feat = np.concatenate((slice_feat, padding), axis=0)
#             if i == 0:
#                 split_feat = slice_feat.reshape(1, length, feat.shape[1])
#             else:
#                 split_feat = np.concatenate([split_feat, slice_feat.reshape(1, length, feat.shape[1])], axis=0)
