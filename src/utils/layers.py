...
class DistanceAdj(Module):
    ...
    def forward(self, batch_size, max_seqlen):
        ...
        # self.dist = torch.from_numpy(squareform(dist)).to('cuda') # GPU
        self.dist = torch.from_numpy(squareform(dist)).to('cpu') # CPU
        self.dist = torch.exp(-self.dist / torch.exp(torch.tensor(1.)))
        # self.dist = torch.unsqueeze(self.dist, 0).repeat(batch_size, 1, 1).to('cuda') # GPU
        self.dist = torch.unsqueeze(self.dist, 0).repeat(batch_size, 1, 1).to('cpu') # CPU
        return self.dist
